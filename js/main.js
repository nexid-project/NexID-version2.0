...
import { showAlert, showConfirm, debounce, initUIEventListeners } from './ui.js';
...
function initializeApp() {
    ...
    initUIEventListeners(DOMElements);
...
// CORRECCIÓN: Esta función ahora solo actualiza los enlaces y los editores, sin recargar todo el perfil.
async function refreshLinksAndEditors() {
    ...
    renderLinksEditor(appState.links);
    updateLivePreview(); // Llama a la vista previa para reflejar el nuevo enlace en el perfil.
}
...
function listenToUserLinks(userId) {
    ...
    appState.subscriptions.links = supabaseClient
        ...
        .on('postgres_changes',	{ event: '*', schema: 'public', table: 'links' },	() => refreshLinksAndEditors())
        .subscribe();
}
...
function renderProfile(profileData, isOwner) {
    ...
    DOMElements.profilePage.className = `p-4 sm:p-6 ${font}`;
...
// NUEVO: Estilos para los botones sociales
...
function getSocialIconForUrl(url) {
    ...
    return info ? socialIcons[info.key] : null; // Usa siempre el objeto de iconos pequeños
}
...
function renderSocialIcons(socials, socialsOrder) {
    ...
    order.forEach(key => {
        ...
        if (username && socialIcons[key]) {
            ...
            link.dataset.socialKey = key; // Add key for reordering
            footer.appendChild(link);
        }
    });
}

// NUEVO: Función para renderizar los botones sociales en el perfil
...
function openSettingsPanel() {
    ...
    setTimeout(() => DOMElements.descriptionInput.dispatchEvent(new Event('input')), 50);
...
// CORRECCIÓN: Esta función ahora construye el objeto de vista previa desde cero,
// leyendo los valores actuales del formulario en lugar de basarse en el estado guardado.
// Esto soluciona los bugs de desincronización.
function updateLivePreview() {
    ...
    document.querySelectorAll('#socials-inputs-container input').forEach(input => {
        if (input.value.trim() !== '') {
            newSocials[input.dataset.social] = input.value.trim();
        }
    });
    ...
    document.querySelectorAll('#contact-inputs input').forEach(input => {
        if (input.value.trim() !== '') {
            newContactInfo[input.dataset.contact] = input.value.trim();
        }
    });
    ...
    const previewData = {
        // Se toman los datos base que no se editan en el panel
        ...appState.profile, 
        // Se sobreescriben con los valores actuales del formulario
        display_name: document.getElementById('display-name-input').value,
        description: document.getElementById('description-input').value,
        background_image_url: document.getElementById('background-image-url-input').value,
        background_overlay_opacity: document.getElementById('background-opacity-slider').value,
        ...
        socials: newSocials,
        socials_order: Object.keys(newSocials), // Se genera un orden temporal para la vista previa
        social_buttons: newSocialButtons,
        ...
        onStart: (evt) => {
            ...
            setTimeout(() => {
                ...
                if (fallbackEl) {
                    ...
                    // CORRECCIÓN DEFINITIVA: Se aplican los estilos computados del elemento original
                    // directamente al clon como estilos en línea. Esto tiene la máxima prioridad
                    // y asegura que los colores del tema (texto, fondo, borde) se mantengan.
                    fallbackEl.style.color = computedStyle.color;
                    fallbackEl.style.backgroundColor = computedStyle.backgroundColor;
                    fallbackEl.style.borderColor = computedStyle.borderColor;
                    fallbackEl.style.boxShadow = computedStyle.boxShadow;

                    // Se mantiene la copia de clases para la estructura (flex, padding, etc.)
                    fallbackEl.className = originalEl.className + ' sortable-fallback';
                    
                    // Se aplica el ancho original para evitar que se redimensione
                    fallbackEl.style.width = `${originalEl.getBoundingClientRect().width}px`;
                }
            }, 0);
        },
        onEnd: () => {
